...
// ConnectionModule provides the NATS connection and JetStreamContext.
// Requires a natsx.Config in the graph, supplied by the service.
var ConnectionModule = fx.Options(
	fx.Provide(NewConnection),
	fx.Invoke(RegisterConnectionLifecycle),
)

// SubscriberModule wires the shared Router, the EventSubscriber (non-queue.*
// subjects), and exposes the Router as a MessageHandlerRegisterer.
...
// WorkerModule wires the QueueSubscriber (queue.* subjects). Requires the
// shared Router already provided by SubscriberModule.
