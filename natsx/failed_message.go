...
// FailedMessage is the GORM model for the failed_tasks DLQ table. It records
// every message that the Worker gave up on after MaxDeliver attempts or a
// terminal error from the handler.
//
// The table name remains "failed_tasks" for backward compatibility with
// existing migrations.
// Operators query / re-publish from here; nothing inside the running app reads
// it on a hot path.
...
// failedMessageRepository persists DLQ rows. Unexported because nothing outside
// this package should know the DLQ table exists.
...
// save writes a DLQ row. Uses FirstOrCreate so duplicate terminal-deliveries
// (e.g. crash between Save and Term) don't error out.
