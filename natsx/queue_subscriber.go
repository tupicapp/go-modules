...
const (
	// defaultMaxDeliver is total delivery attempts (1 original + 10 retries).
	// Must equal len(backoffSchedule)+1 so the final attempt triggers DLQ.
	defaultMaxDeliver = 11

	// defaultAckWait is how long JetStream waits for any response (Ack/Nak)
	// before treating the handler as stuck and redelivering. Must exceed the
	// slowest expected handler execution time.
	defaultAckWait = 30 * time.Second
)

// backoffSchedule defines the wait before each retry. Spreads 10 retries
// over ~5.5 days so a multi-day downstream outage does not immediately DLQ.
//
// NOTE: this only applies when the service is UP but handlers are failing.
// If the service process is completely down, messages wait undelivered in
// the JetStream stream — stream MaxAge is what prevents loss in that case.
...
// QueueSubscriber subscribes to queue.* subjects and dispatches each task to the Router.
// Terminal failures (apperror) and exhausted MaxDeliver are written to the failed_messages DLQ and Term'd; transient
// failures are Nak'd for JetStream retry.
...
// Start subscribes to every registered queue.* subject.
// Each subject gets its own durable consumer so JetStream load-balances across replicas naturally.
...
// drain completes a warning is logged so downstream lifecycle hooks are not
// blocked.
...
func (w *QueueSubscriber) handleMessage(msg *natslib.Msg) {
	...
	// If metadata is unavailable we cannot determine delivery count.
	// Treat as terminal to guarantee a DLQ row rather than silently losing
	// the message when JetStream exhausts MaxDeliver at the broker level.
	if metaErr != nil {
...
// deadLetter writes the failed task to the DLQ table. Failures here are
// logged but not propagated — losing the DLQ row is worse than continuing,
// and JetStream MaxDeliver guarantees the message has already been retried
// the allowed number of times.
func (w *QueueSubscriber) deadLetter(e *envelope, subject string, attempts int, handlerErr error) {
	id, err := uuid.Parse(e.ID)
	if err != nil {
		// Envelope ID was unparseable; mint a fresh one so we don't lose
		// the DLQ entry. Should never happen since the outbox writes UUIDv7s.
