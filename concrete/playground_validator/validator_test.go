package playground_validator_test
...
func (s *ValidatorSuite) TestValidate_ValidStruct_ReturnsNil() {
	err := playground_validator.NewPlayground().Validate(signup{Email: "a@b.com", Locale: "en-US"})
	s.NoError(err)
}

func (s *ValidatorSuite) TestValidate_RequiredField_ReturnsValidationError() {
	err := playground_validator.NewPlayground().Validate(signup{Locale: "en-US"})
...
func (s *ValidatorSuite) TestValidate_InvalidEmail_ReturnsEmailMessage() {
	err := playground_validator.NewPlayground().Validate(signup{Email: "not-an-email", Locale: "en-US"})
...
func (s *ValidatorSuite) TestValidate_InvalidLocale_ReturnsLocaleMessage() {
	err := playground_validator.NewPlayground().Validate(signup{Email: "a@b.com", Locale: "english"})
...
func (s *ValidatorSuite) TestValidate_ReportsEveryViolation() {
	err := playground_validator.NewPlayground().Validate(signup{})
...
func (s *ValidatorSuite) TestWithAlias_RegistersTagAlias() {
	...
	v := playground_validator.NewPlayground(playground_validator.WithAlias("slug", "required,min=3"))
...
func (s *ValidatorSuite) TestWithValidation_RegistersCustomTag() {
	...
	v := playground_validator.NewPlayground(playground_validator.WithValidation("even", func(fl playgroundLib.FieldLevel) bool {
