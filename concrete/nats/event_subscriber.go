...
// defaultEventAckWait is how long NATS waits for an Ack on an event consumer before redelivering. Must exceed the
// slowest expected handler execution.
...
// drain completes a warning is logged and the function returns so downstream lifecycle hooks are not blocked.
...
func (s *EventSubscriber) handleMessage(msg *natslib.Msg) {
	...
	if err := s.handler.Handle(s.cancelCtx, msg.Subject, m); err != nil {
		if apperror.IsAppError(err) {
			...
			sentry.Capture(err, map[string]string{"transport": "nats", "subject": msg.Subject})
