...
// Config selects the persistence driver and carries its settings. Test enables the smaller test connection pool and
// skips re-pinging a pool that is already in use (suite-level transactions hold a connection open).
type Config struct {
	Driver   string
