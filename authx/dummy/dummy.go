// Package dummy provides an Authenticator that decodes the bearer token as a
// base64-encoded JSON Actor.
// Useful in tests where no real IAM server is available.
package dummy
...
// Authenticator decodes the bearer token as a base64-encoded JSON Actor.
// For user actors it fetches the full user record via the service-supplied
// findUser function so callers receive a populated user entity (matching the
// behavior of the IAM authenticator).
