...
//
// The package is generic over the service's user entity: JWT validation, claims parsing, and actor construction are
// shared, while user resolution (find-or-create against the service's user store) is delegated to a service-supplied
// UserResolver.
//
// # Token kinds
//
...
//
//   - Service-account tokens carry service_account_client_id. They produce a service actor and never touch the user
//     store — internal machine clients have no user entity.
...
//
// Keycloak access tokens may omit realm roles. Authenticate builds the actor from whatever roles the token carries (so
// most requests pay nothing). Routes that actually need accurate roles — admin routes — call EnsureRoles, which fetches
// them from the userinfo endpoint when the actor has none. The HTTP layer wires this as an explicit middleware on the
// admin route group (see echo.EnsureRoles); there is no hidden per-request flag.
package iam
...
// UserResolver finds or provisions the service's user entity from validated token claims. Implemented by each service
// (e.g. find-or-upsert with domain event publication, unique-username resolution).
...
// Authenticator validates Bearer tokens against the Tupic IAM (Keycloak) JWKS endpoint and resolves the service's user
