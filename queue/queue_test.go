...
func (s *QueueSuite) TestEnqueue_StoresTaskAsEvent() {
	...
	err := q.Enqueue(context.Background(), stubTask{subject: "queues.validate-asset", version: "2"})
	s.Require().NoError(err)

	// The task must reach the outbox preserving its subject/version shape.
	s.Require().NotNil(ob.stored)
	s.Equal("queues.validate-asset", ob.stored.Subject())
...
func (s *QueueSuite) TestEnqueue_PropagatesOutboxError() {
	...
	err := q.Enqueue(context.Background(), stubTask{subject: "queues.x", version: "1"})
	s.Require().Error(err)
	s.ErrorIs(err, boom)
}
